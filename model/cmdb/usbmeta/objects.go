...
func (this *Vendor) Create() (int64, error) {
	return dataStore.Exec(`Replace`, this)
}
...
func Load(usb *peripheral.Usb) (error) {
	...
	if stmt, err := dataStore.NamedStmt(`Replace`, vendor); err != nil {
		return err
	} else {
		createVendor = tx.NamedStmt(stmt)
	}

	if stmt, err := dataStore.NamedStmt(`Replace`, product); err != nil {
		return err
	} else {
		createProduct = tx.NamedStmt(stmt)
	}

	if stmt, err := dataStore.NamedStmt(`Replace`, class); err != nil {
		return err
	} else {
		createClass = tx.NamedStmt(stmt)
	}

	if stmt, err := dataStore.NamedStmt(`Replace`, subClass); err != nil {
		return err
	} else {
		createSubClass = tx.NamedStmt(stmt)
	}

	if stmt, err := dataStore.NamedStmt(`Replace`, protocol); err != nil {
		return err
	} else {
