	...
	for _, f := range files {
		...
		if err = os.MkdirAll(filepath.Dir(f), LogDirMode); err == nil {
			if h, err = os.OpenFile(f, LogFileFlags, LogFileMode); err == nil {
				this.writers = append(this.writers, h)
...
func (this *MultiWriter) AddSyslog(proto, raddr, tag string, pri srslog.Priority) {
	...
	if syslog, err := srslog.Dial(proto, raddr, pri, tag); err == nil {
		this.writers = append(this.writers, syslog)
	} else {
		log.Printf("%v", err)
	}
}
...
func (this *MultiWriter) Write(p []byte) (n int, err error) {
	...
	if fail > 0 || short > 0 {
		err = fmt.Errorf("%d write failures, %d short writes", fail, short)
	}
